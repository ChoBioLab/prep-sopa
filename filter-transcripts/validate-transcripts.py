...
def setup_logger(log_dir: Path) -> logging.Logger:
    ...
    file_handler = logging.FileHandler(
        log_dir / f"validation_log_{timestamp}.txt", encoding="utf-8"
    )
...
def analyze_csv_in_chunks(
    file_path: Path, logger: logging.Logger, chunk_size: int = 1_000_000
) -> Dict:
...
def validate_filtered_transcripts(filtered_dir: Path, logger: logging.Logger) -> Dict:
    ...
    for file_type, file_path in files.items():
        ...
        try:
            if file_type.endswith("csv"):
                metrics["analysis"][file_type] = analyze_csv_in_chunks(
                    file_path, logger
                )
            else:
            ...
            logger.info(
                f"  Total rows: {metrics['analysis'][file_type]['total_rows']:,}"
            )
            logger.info(
                f"  Rows with QV >= 20: {metrics['analysis'][file_type]['qv_ge_20']:,}"
            )
            logger.info(
                f"  QV range: {metrics['analysis'][file_type]['min_qv']:.2f} - {metrics['analysis'][file_type]['max_qv']:.2f}"
            )
    ...
    for fmt in ["csv", "parquet"]:
        ...
        if orig_key in metrics["analysis"] and filt_key in metrics["analysis"]:
            ...
            logger.info(
                f"  Matches expected: {orig_metrics['qv_ge_20'] == filt_metrics['total_rows']}"
            )
            logger.info(f"  All QV >= 20: {filt_metrics['min_qv'] >= 20}")

            size_ratio = (
                metrics["file_sizes"][filt_key] / metrics["file_sizes"][orig_key]
            )
