...
def setup_transcript_directories(data_path: Path) -> None:
    ...
    if existing_transcripts:
        logger.info(
            "Found existing transcript files in original-transcripts. Checking filtered transcripts..."
        )
        ...
        if filtered_dir.exists():
            for transcript_file in data_path.glob("transcripts.*"):
                if transcript_file.is_file():
                    logger.info(
                        f"Removing existing {transcript_file.name} from data_path"
                    )
            ...
            for transcript_file in filtered_dir.glob("transcripts.*"):
                if transcript_file.is_file():
                    ...
                    logger.info(
                        f"Copying {transcript_file.name} from qv20-filtered-transcripts to data_path"
                    )
